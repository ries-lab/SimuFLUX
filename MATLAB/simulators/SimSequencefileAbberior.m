classdef SimSequencefileAbberior<Simulator
    ...
    methods
        ...
        function makepatterns(obj,psfs,phasemasks)
            if nargin>1
                dips('psfs explicitely input to makepatterns currently ignored, psfs are based on sequence file')
            end
            psf=obj.psfvec;
            itrs=obj.sequence.Itr;
            for k=1:length(itrs)
                [parg,estimator]=getAbberiorPattern(itrs(k),obj.sequence);
                obj.definePattern("itr"+k, psf, parg{:});  
                obj.estimators(k)=estimator;
            end
        end
        function out=runSequenceintern(obj)
            ...
            maxiter=length(itrs);
            starttime=obj.time;
            xestabs=obj.posgalvo+obj.posEOD; %first estimate: pattern center
            ...
            while numitr<loclimit && stickinesscounter<stickiness && ~abortphot
                ...
                if photsum>0 %no photon detected: we don't write localization
                    ...
                    out.loc.xnm(loccounter,1)=xestabs(1);out.loc.ynm(loccounter,1)=xestabs(2);out.loc.znm(loccounter,1)=xestabs(3);
